...
本地两个服务已启用 `policy_version=oasis-static-v4`。每次自动 PR 评审固定加载 `prompts/oasis-review.md`，采集完整提交日志及 PR 说明；缺需求描述时根据 commit log 和 diff 推断。输出“修改概述 / 代码问题 / 待确认项”三节中文正文，并由控制器生成结论及合入建议。仅“通过”且缺陷等级低于门限可进入合入检查；“有条件通过／不通过／证据不足”均阻断。
